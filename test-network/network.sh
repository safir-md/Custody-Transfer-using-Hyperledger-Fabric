...
function createOrgs() {
  ...
  infoln "Generate CCP files for Org1 and Org2 and org4 and org5 and org6"
...
function networkUp() {
  ...
  if [ "${DATABASE}" == "couchdb" ]; then
    COMPOSE_FILES="${COMPOSE_FILES} -f ${COMPOSE_FILE_COUCH} -f ${COMPOSE_FILE_CA}"
  fi
